...
class SparseMixtureGaussianPolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name, n_mixture=3):
        ...
        Policy.__init__(
            ...
            num_actiondist_params=(action_space.dim * 2+1)*n_mixture,
    ...
    def _make_actiondist_ops(self, obsfeat_B_Df):
        ...
        with nn.variable_scope('out'):
            mean_layer = nn.AffineLayer(net.output, net.output_shape, ((self.action_space.dim+1)*self.n_mixture,),
                                        initializer=np.zeros((net.output_shape[0], (self.action_space.dim+1)*self.n_mixture)))
            assert mean_layer.output_shape == ((self.action_space.dim+1)*self.n_mixture,)
        means_B_Da = mean_layer.output[:,:self.action_space.dim*self.n_mixture]
        weights_B_Da = sparsemax_theano.sparsemaxdist(mean_layer.output[:,self.action_space.dim*self.n_mixture:])

        # Action distribution log standard deviations are parameters themselves
        logstdevs_1_Da = nn.get_variable('logstdevs_1_Da', np.full((1, self.action_space.dim*self.n_mixture), self.cfg.init_logstdev),
                                         broadcastable=(True, False))
        stdevs_1_Da = self.cfg.min_stdev + tensor.exp(
            logstdevs_1_Da)  # minimum stdev seems to make density / kl computations more stable
        stdevs_B_Da = tensor.ones(shape=(means_B_Da.shape[0],self.action_space.dim*self.n_mixture)) * stdevs_1_Da  # "broadcast" to (B,Da)

        actiondist_B_Pa = tensor.concatenate([means_B_Da, weights_B_Da, stdevs_B_Da], axis=1)
    ...
    def _extract_actiondist_params(self, actiondist_B_Pa):
        ...
        means_B_Da = actiondist_B_Pa[:, :adim*n_mixture].reshape((-1,adim,n_mixture))
        weights_B_Da = actiondist_B_Pa[:, adim*n_mixture:(adim+1)*n_mixture]
        stdevs_B_Da = actiondist_B_Pa[:, (adim+1)*n_mixture:].reshape((-1,adim,n_mixture))
        return means_B_Da, weights_B_Da, stdevs_B_Da
    ...
    def _make_actiondist_logprob_ops(self, actiondist_B_Pa, input_actions_B_Da):
        ...
        return thutil.mixture_gaussian_log_density(means_B_Da, stdevs_B_Da, weights_B_Da, input_actions_B_Da)
    ...
    def _sample_from_actiondist(self, actiondist_B_Pa, deterministic):
        ...
        adim = self.action_space.dim
        n_mixture = self.n_mixture
        means_B_Da = np.reshape(actiondist_B_Pa[:, :n_mixture*adim],(-1,adim,n_mixture))
        weights_B_Da = actiondist_B_Pa[:, n_mixture*adim:n_mixture*(adim+1)]
        stdevs_B_Da = np.reshape(actiondist_B_Pa[:, n_mixture*(adim+1):],(-1,adim,n_mixture))

        if deterministic:
            mixture_maximum_B = np.argmax(weights_B_Da,axis=1)
            return means_B_Da[range(batch_size),:,mixture_maximum_B]

        mixture_sample_B = util.sample_cats(weights_B_Da)
        ...
        return (stdnormal_B_Da * stdevs_B_Da[range(batch_size),:,mixture_sample_B]) + means_B_Da[range(batch_size),:,mixture_sample_B]

    def _compute_actiondist_entropy(self, actiondist_B_Pa):
        _, weights_B_Da, stdevs_B_Da = self._extract_actiondist_params(actiondist_B_Pa)
        return util.mixture_gaussian_entropy(weights_B_Da, stdevs_B_Da)

    def compute_actiondist_mean(self, obsfeat_B_Df):
        batch_size = obsfeat_B_Df.shape[0]

        actiondist_B_Pa = self._compute_action_dist_params(obsfeat_B_Df)
        means_B_Da, weights_B_Da, _ = self._extract_actiondist_params(actiondist_B_Pa)
        mixture_sample_B = util.sample_cats(weights_B_Da)

        return means_B_Da[range(batch_size),:,mixture_sample_B]

MixtureGaussianPolicyConfig = namedtuple('MixtureGaussianPolicyConfig', 'hidden_spec, min_stdev, init_logstdev, enable_obsnorm')
class MixtureGaussianPolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name, n_mixture=3):
        assert isinstance(cfg, MixtureGaussianPolicyConfig)
        assert isinstance(obsfeat_space, ContinuousSpace) and isinstance(action_space, ContinuousSpace)
    ...
    def _make_actiondist_ops(self, obsfeat_B_Df):
        ...
        with nn.variable_scope('out'):
            ...
            assert mean_layer.output_shape == ((self.action_space.dim+1)*self.n_mixture,)
        means_B_Da = mean_layer.output[:,:self.action_space.dim*self.n_mixture]
        weights_B_Da = tensor.nnet.softmax(mean_layer.output[:,self.action_space.dim*self.n_mixture:])
...
class GibbsPolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name, scale=1):
...
SparsePolicyConfig = namedtuple('SparsePolicyConfig', 'hidden_spec, enable_obsnorm')
class SparsePolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name, scale=1):
        assert isinstance(cfg, SparsePolicyConfig)
        assert isinstance(obsfeat_space, ContinuousSpace) and isinstance(action_space, FiniteSpace)
        self.cfg = cfg
        self.scale = scale
        Policy.__init__(
    ...
    def _make_actiondist_ops(self, obsfeat_B_Df):
        ...
        with nn.variable_scope('out'):
            ...
            assert out_layer.output_shape == (self.action_space.size,)
        scores_B_Pa = out_layer.output # unnormalized (unshifted) log probability
        actiondist_B_Pa = scores_B_Pa - self.scale*sparsemax_theano.sparsemax(scores_B_Pa/self.scale) # log probability
        return actiondist_B_Pa
    ...
    def _sample_from_actiondist(self, actiondist_B_Pa, deterministic):
        ...
        for i in range(actiondist_B_Pa.shape[0]):
            probs_B_A[i,:],_,_,_ = sparsemax_theano.simplex_projection(actiondist_B_Pa[i,:]/self.scale)
    ...
    def _compute_actiondist_entropy(self, actiondist_B_Pa):
        ...
        for i in range(actiondist_B_Pa.shape[0]):
            probs_B_A[i,:],_,_,_ = sparsemax_theano.simplex_projection(actiondist_B_Pa[i,:]/self.scale)
        return self.scale*util.categorical_sparse_entropy(probs_B_A)
