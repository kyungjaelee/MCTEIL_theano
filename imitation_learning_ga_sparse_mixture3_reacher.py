...
def load_dataset(filename, limit_trajs, data_subsamp_freq):
    ...
    assert exobs_Bstacked_Do.shape[0] == exa_Bstacked_Da.shape[0] == ext_Bstacked.shape[
        0]  # == np.ceil(exlen_B.astype(float)/data_subsamp_freq).astype(int).sum() > 0

    print 'Subsampled data every {} timestep(s)'.format(data_subsamp_freq)
    print 'Final dataset size: {} transitions (average {} per traj)'.format(exobs_Bstacked_Do.shape[0], float(
        exobs_Bstacked_Do.shape[0]) / dset_size)

    return exobs_Bstacked_Do, exa_Bstacked_Da, ext_Bstacked


def main(cmd):
    # with open('./pipelines/im_ga_mixture5_ant.yaml', 'r') as f:
    #     spec = yaml.load(f)
    ...
    if isinstance(mdp.action_space, policyopt.ContinuousSpace):
        if args.policy_type == 'Gaussian':
            ...
            policy_cfg = rl.SparseMixtureGaussianPolicyConfig(
                ...
                init_logstdev=0.,
                enable_obsnorm=enable_obsnorm)
            policy = rl.SparseMixtureGaussianPolicy(policy_cfg, mdp.obs_space, mdp.action_space,
                                                    'GaussianSparseMixturePolicy', n_mixture=args.n_mixture)
    else:
    ...
    for i in xrange(args.max_iter):
        ...
        if args.plot_freq != 0 and i % args.plot_freq == 0:
            ...
            _, ax = plt.subplots()
            idx1, idx2 = 0, 1
            range1 = (min(exdata_N_Doa[:, idx1].min(), pdata_M_Doa[:, idx1].min()),
                      max(exdata_N_Doa[:, idx1].max(), pdata_M_Doa[:, idx1].max()))
            range2 = (min(exdata_N_Doa[:, idx2].min(), pdata_M_Doa[:, idx2].min()),
                      max(exdata_N_Doa[:, idx2].max(), pdata_M_Doa[:, idx2].max()))
...
with open('./pipelines/im_ga_sparse_mixture3_reacher.yaml','r') as f:
    spec = yaml.load(f)
...
# assert not os.listdir(checkptdir), 'Checkpoint directory {} is not empty!'.format(chcheckptdireckptdir)
